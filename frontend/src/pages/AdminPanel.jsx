...
const AdminPanel = () => {
    ...
    const handleUpdateUser = async (e) => {
        ...
        try {
            ...
            setEditUser(null);
            fetchData();
        } catch (err) { alert(err.response?.data?.error || err.response?.data?.message || 'Failed to update user') }
    };
    ...
    const fetchSalaryHistory = async (userId) => {
        try {
            const { data } = await api.get(`/admin/salary-history/${userId}`);
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-700">
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-50">
                                        ...
                                        <th className="pb-6 px-4">Salary</th>
                                    ...
                                    {users.slice(0, 5).map((user) => (
                                        <tr key={user.id} className="group hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="py-6 px-4">
                                                 ...
                                                 <p className="text-[9px] font-bold text-slate-400 uppercase mt-1 tracking-wider">{user.role}</p>
                                            </td>
                                            <td className="py-6 px-4">
                                                <p className="font-black text-slate-900 text-sm leading-none">${(user.salary || 0).toLocaleString()}</p>
                                            </td>
                                            <td className="py-6 px-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <button onClick={() => fetchSalaryHistory(user.id)} className="p-2.5 text-slate-300 hover:text-amber-600 hover:bg-amber-50 rounded-xl transition-all" title="Salary History"><History size={18} /></button>
                                                    <button onClick={() => { setEditUser(user); setActiveModal('editUser'); }} className="p-2.5 text-slate-300 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all"><Edit3 size={18} /></button>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {users.map((user) => (
                                <tr key={user.id} className="hover:bg-slate-50/80 transition-all">
                                    ...
                                    <td className="py-6 px-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button onClick={() => fetchSalaryHistory(user.id)} className="p-3 text-slate-300 hover:text-amber-600 hover:bg-amber-50 rounded-2xl transition-all" title="Salary History"><History size={20} /></button>
                                            <button onClick={() => { setEditUser(user); setActiveModal('editUser'); }} className="p-3 text-slate-300 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl transition-all" title="Edit User"><Edit3 size={20} /></button>
                                            <button onClick={() => handleDeleteUser(user.id)} className="p-3 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all" title="Delete User"><Trash2 size={20} /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
            ...
            <DetailModal isOpen={activeModal === 'salaryHistory'} onClose={() => setActiveModal(null)} title="Compensation Audit Trail">
                    ...
                    <form onSubmit={handleUpdateUser} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-1.5 md:col-span-2">
                                ...
                                <input required value={editUser.name} onChange={e => setEditUser({...editUser, name: e.target.value})} className="w-full p-4.5 bg-slate-50 border-2 border-transparent rounded-2xl font-bold focus:border-indigo-600 outline-none" />
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email Address</label>
                                <input required type="email" value={editUser.email} onChange={e => setEditUser({...editUser, email: e.target.value})} className="w-full p-4.5 bg-slate-50 border-2 border-transparent rounded-2xl font-bold focus:border-indigo-600 outline-none" />
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Role Permissions</label>
                                <select value={editUser.role} onChange={e => setEditUser({...editUser, role: e.target.value})} className="w-full p-4.5 bg-slate-50 border-2 border-transparent rounded-2xl font-black uppercase tracking-widest focus:border-indigo-600 outline-none">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <select value={editUser.department_id} onChange={e => setEditUser({...editUser, department_id: e.target.value})} className="w-full p-4.5 bg-slate-50 border-2 border-transparent rounded-2xl font-black uppercase tracking-widest focus:border-indigo-600 outline-none">
                                    {departments.map(dept => (
                             ...
                             <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking_widest ml-1">Reporting Manager</label>
                                <select value={editUser.manager_id || ''} onChange={e => setEditUser({...editUser, manager_id: e.target.value})} className="w-full p-4.5 bg-slate-50 border-2 border-transparent rounded-2xl font-black uppercase tracking-widest focus:border-indigo-600 outline-none">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <input required type="number" value={editUser.salary} onChange={e => setEditUser({...editUser, salary: e.target.value})} className="w-full p-4.5 bg-slate-50 border-2 border-transparent rounded-2xl font-bold focus:border-indigo-600 outline-none" />
                            </div>
                        </div>
                        <button type="submit" className="w-full bg-slate-900 text-white p-5 rounded-3xl font-black hover:bg-black transition-all shadow-xl shadow-slate-200 flex items-center justify-center gap-3">
                            <Save size={18} />
                            Commit Updates
                        </button>
                    </form>
                ...
                <form onSubmit={handleCreateUser} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-1.5">
                         ...
                         <div className="space-y-1.5 md:col-span-2">
                              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Annual Salary ($)</label>
                              <input required type="number" value={newUser.salary} onChange={e => setNewUser({...newUser, salary: e.target.value})} className="w-full p-4.5 bg-slate-50 border-2 border-transparent rounded-2xl font-bold focus:border-indigo-600 outline-none" placeholder="e.g. 75000" />
                         </div>
                          <div className="space-y-1.5 md:col-span-2">
