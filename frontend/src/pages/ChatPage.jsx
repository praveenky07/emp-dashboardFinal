...
const ChatPage = () => {
    ...
    const [selectedChat, setSelectedChat] = useState(null); // { id: convId, name, type: 'direct' | 'group', otherUserId? }
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setGroups(convRes.data.filter(c => c.type === 'group'));
            } catch (err) {
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            try {
                const res = await api.get(`/chat/conversations/${selectedChat.id}/messages`);
        ...
        if (selectedChat.id) fetchHistory();
        setMobileSidebarOpen(false); // Close on mobile after selection

        socket.emit('join_room', `conversation_${selectedChat.id}`);

        const handleNewMessage = (data) => {
            console.log('[SOCKET] Received Message:', data);
            if (String(data.conversationId) === String(selectedChat.id)) {
    ...
    const handleSendMessage = (e, fileUrl = null, fileType = null) => {
        ...
        if (selectedChat.type === 'direct') {
            messageData.receiverId = selectedChat.otherUserId;
            socket.emit('private_message', messageData);
            setMessages(prev => [...prev, { ...messageData, sender_id: currentUser.id, created_at: new Date().toISOString() }]);
        } else {
    ...
    const handleSelectUser = async (u) => {
        try {
            ...
            setSelectedChat({ id: res.data.id, name: u.name, type: 'direct', otherUserId: u.id });
    ...
    const handleCreateGroupSubmit = async (e) => {
        ...
        try {
            ...
            setSelectedChat({ id: res.data.id, name: groupFormName, type: 'group' });
                    ...
                    {activeTab === 'private' ? (
                        filteredUsers.map(u => (
                            <button 
                                ...
                                className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all group ${selectedChat?.otherUserId === u.id ? 'bg-white border border-slate-100 shadow-md translate-x-1' : 'hover:bg-white/50'}`}
                            >
                        ...
                        <div className="space-y-4">
                            {(currentUser.role === 'Admin' || currentUser.role === 'admin' || currentUser.role === 'manager' || currentUser.role === 'Manager') && (
                                <button onClick={() => setIsGroupModalOpen(true)} className="w-full p-4 mb-2 bg-indigo-50 text-indigo-700 font-bold rounded-2xl border border-indigo-100 hover:bg-indigo-100 transition-all text-sm flex items-center justify-center gap-2">
                                    + Create New Group
                                </button>
                            )}
                            {groups.map(g => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 text-left overflow-hidden">
                                        ...
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate">Company Group</p>
                                    </div>
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="p-6 border-b border-slate-50 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black text-lg">
                                    {selectedChat.type === 'direct' ? selectedChat.name[0] : <Hash size={24} />}
                                </div>
                                <div>
                    ...
                    {isGroupModalOpen && (
                        ...
                        >
                            <motion.div 
                                ...
                                className="bg-white w-full max-w-md rounded-[32px] p-8 shadow-2xl flex flex-col max-h-[85vh]"
                            >
                                <div className="flex justify-between items-center mb-6">
                                    <h2 className="text-xl font-black text-slate-900">Create New Group</h2>
                                    <button onClick={() => setIsGroupModalOpen(false)} className="text-slate-400 hover:text-red-500 transition-colors"><X size={24} /></button>
                                </div>
                                <form onSubmit={handleCreateGroupSubmit} className="flex flex-col flex-1 overflow-hidden">
                                    <div className="mb-6 shrink-0">
                                        <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Group Name</label>
                                        <input 
                                            type="text" 
                                            required
                                            value={groupFormName}
                                            ...
                                            className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all font-medium text-slate-900 outline-none"
                                        />
                                    </div>
                                    <div className="flex-1 overflow-y-auto min-h-0 pr-2 custom-scrollbar">
                                        <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-3">Select Members ({groupFormMembers.length})</label>
                                        <div className="space-y-2">
                                            {users.map(u => (
                                                <label key={u.id} className="flex items-center p-3 border border-slate-100 rounded-xl hover:bg-slate-50 cursor-pointer transition-colors group">
                                                    <input 
                                                        type="checkbox" 
                                                        checked={groupFormMembers.includes(u.id)}
                                                        onChange={(e) => {
                                                            if (e.target.checked) setGroupFormMembers(prev => [...prev, u.id]);
                                                            else setGroupFormMembers(prev => prev.filter(id => id !== u.id));
                                                        }}
                                    ...
                                    <button 
                                        ...
                                        className="w-full py-4 mt-6 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 active:scale-[0.98] transition-all flex items-center justify-center shrink-0"
                                    >
                                        {groupActionLoading ? 'Creating...' : 'Launch Group'}
                                    </button>
                                </form>
                            </motion.div>
                        </motion.div>
